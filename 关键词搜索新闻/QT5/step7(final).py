...
class CrawlWindow(QWidget):
    ...
    def table_init(self):
        ...
        self.table.setHorizontalHeaderLabels(['新闻链接', '渠道来源', '发布日期', '新闻摘要', '关键词'])
    ...
    def btn_slot(self, btn):
        ...
        if btn == self.start_btn:
            ...
            self.crawl_thread.render(self.movie_name.text())
...
class CrawlThread(QThread):
    ...
    def render(self, word):
        ...
        url_list, pubtime_list, sitename_list, summary_list = searchNews.xinhua_news(word)

        for i in range(len(url_list)):
            self.result_signal.emit(str(url_list[i]), str(pubtime_list[i]), str(sitename_list[i]), str(summary_list[i]), str(2))
            print(url_list[i], pubtime_list[i], sitename_list[i], summary_list[i], str(2))
    ...
    #         print(img_list[i], name_list[i], star_list[i], time_list[i], score_list[i])

...
class commentTodoList(Resource):  # 评论
    ...
    def get(self):
        try:
            ...
            cursor = args['cursor']
            info = {"info": comment.apiComment(comment.getID(aweme_id), cursor)}
...
class favoriteTodoList(Resource):  # 喜欢
    ...
    def get(self):
        try:
            ...
            info = {"info": favorite.apiFavorite(follower.getParam(user_id), max_cursor)}
...
class followerTodoList(Resource):  # 粉丝
    ...
    def get(self):
        try:
            ...
            info = {"info": follower.apiFollower(follower.getParam(user_id), max_time)}
...
class getaweme_id(Resource):  # 获取视频id
    def get(self):
        ...
        args = parser_put.parse_args()
        url = args['url']
        aweme_id = {"aweme_id": comment.getID(url)}
...
class user_id(Resource):  # 获取用户id
    def get(self):
        ...
        args = parser_put.parse_args()
        link = args['link']
        user_id = {"user_id": follower.getParam(link)}
...
class index(Resource):  # 主页
    def get(self):
        return 'This is DouYinAPI API', 200

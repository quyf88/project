...
class Db:
    ...
    def update(self, msg):
        conn, c = self.get_conn_c()
        try:
            c.execute('''UPDATE poison_data SET timestamp = ? WHERE productId = ?;''',
                      [str(time.time()), msg['productId']])
    ...
    def insert_detail(self, msg):
        ...
        try:
            ...
            c.execute(sql_string,
                      [result['name'], result['productId'], result['price'], result['size_list'], result['brand_id'],
                       result['color'], result['articleNumber'], result['authPrice'], result['sellDate'],
                       result['exchangeDesc'], result['soldNum'], result['text_string'], result['img_list'],
                       result['size_price']])
            c.close()
            conn.commit()
            conn.close()

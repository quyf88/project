...
# # # async def fetch_json(session,productId):
# # #     url_dic = {
...
# # #                 async with session.get(self.make_url(url_dic,end_url),headers = self.header ,verify_ssl=False) as response:
...
class Crakeme:
    def __init__(self):
        ...
        self.cookie = {
            ...
            #"duToken":"3a30b9a6%7C14460122%7C1545383869%7C982217a9cc324b83"
        }
...
async def fetch(productId):
    url_dic = {
    "productId" : str(productId),
    "isChest"  : "1"
    }
    end_url = "/product/detail?"
    async with aiohttp.ClientSession(cookies=a.cookie) as session:
        async with session.get(a.make_url(url_dic,end_url),headers = a.header) as r:
            # print(r.headers)
            # print(r.status)
            # print(await r.json())
            return await r.json()
...
r = loop.run_until_complete(fetch(10896))
loop.close()
...
w = r
msg = {"productId":w['data']['detail']['productId'],"json_data":w,"img_list":get_json_img(w)}
print(time.time())
a.insert_detail(msg)
a.update(msg)
a.query_diff_exists(msg)
a.insert_diff_table(msg)
print(time.time())

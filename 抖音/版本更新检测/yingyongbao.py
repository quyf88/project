...
print(f'当前脚本绝对路径:{os.path.abspath(__file__)}')  # 当前文件绝对路径
PATH = os.getcwd()  # 文件路径
...
def log_init():
    ...
    # 输出日志至文件
    path = PATH + '/log/'  # 日志保存路径
    if not os.path.exists(path):
        os.mkdir(path)
    filename = path + datetime.datetime.now().strftime('%Y-%m-%d') + '.txt'
...
class DouYinVersion:
    ...
    def version(self):
        url = 'https://sj.qq.com/myapp/detail.htm?apkName=com.ss.android.ugc.aweme'
        rsp = requests.get(url, headers=self.headers, timeout=10)
        ver = re.findall(r'<div class="det-othinfo-data">V(.*?)</div>', rsp.text)
        if not ver:
    ...
    def red_config(self):
        ...
        version = cp.get('Version', 'yingyongbao_version')  # 读取值
        return version
    ...
    def set_config(self, new_version):
        ...
        cp = ConfigParser()  # 实例化
        cp.read(self.ini_name)  # 读取文件
        cp.set('Version', 'yingyongbao_version', new_version)  # 修改数据
        # 写入新数据

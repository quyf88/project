...
class Spider:
    ...
    def slide(self):
        ...
        while True:
            ...
            try:
                # 非直播
                comment = WebDriverWait(self.driver, 10, 0.1, AttributeError).until(
                            EC.presence_of_all_elements_located((By.ID, 'com.ss.android.ugc.aweme:id/egv')))
            except:
                # 直播
                # comment = WebDriverWait(self.driver, 10, 0.1, AttributeError).until(
                #     EC.presence_of_all_elements_located((By.ID, 'com.ss.android.ugc.aweme:id/egx')))
            ...
            time.sleep(3)
            # 判断数据是否刷新出来
            if not self.driver.find_elements_by_id('com.ss.android.ugc.aweme:id/efa'):
                self.driver.keyevent(4)
                time.sleep(2)
                self.driver.keyevent(4)
                time.sleep(1)
